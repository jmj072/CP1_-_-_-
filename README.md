...
> ✔ 강남구 지역의 아파트 실거래가를 예측하는 모델을 생성하여, **해당 지역에 대한 인사이트**를 얻고 **정확한 모델**을 생성할 수 있는지 평가하고자함  
> ✔ 강남구의 <U>아파트 정보, 인구학적 정보, 주변 지리적 정보, 경제지표</U>가 강남구 **아파트 실거래가에 영향**을 줄 것으로 가정하여 프로젝트를 진행함
...
- TOP 10 브랜드 아파트 특성 생성

- 인구학적 정보 및 경제지표 특성들의 PCA 진행
  - 상관관계 분석 결과, 인구학적 정보 및 경제지표 특성 간의 연관 관계 확인 (상관계수 ≥ 0.3)
  - 다중 공산성 문제를 방지하기 위하여, 특성들의 PCA 진행 ⇒ PC1의 누적 기여율이 80%이므로, PC1만 선택
...
- Grid Search CV를 이용해 Hyper Parameter tuning 진행, 모델 성능을 개선함
	...
	- y_true와 y_pred 값의 기울기가 거의 0.5를 형성 ⇒ 오차가 매우 작음
...
> 예측모델의 오차를 고려하였을 때, 예측이 정확한 것으로 생각됨
...
- 특성 중 아파트 평수에 대한 특성이 거래 금액 예측에 가장 큰 영향을 줌 
  - 평수와 관련된 방의 개수, 화장실 개수 등 구체적인 특성으로 분리할 수 있어, 분리하여 알아볼 필요가 있음

- 인구학적 정보와 경제상황에 대한 특성을 PCA하여 모델에 적용
  -  인구학적 정보와 경제 상황 중 어떤 특성이 더 많이 아파트 가격에 영향을 주는지 알 수 없음

- 정부의 부동산 정책에 대한 영향을 배제하기 어려움


### 개선 사항
- 아파트 평수에 대한 특성을 구체적인 특성으로 바꾸어 모델 학습 진행할 것
...
- 프로젝트 초기단계에서 구체적인 목표 없이 대량의 데이터를 수집했음
  - 30만 여개의 데이터를 목적없이 다루다보니, API 이용과 의미 파악이 쉽지않았음
...
[[프로젝트 상세] PPT File 링크](https://drive.google.com/file/d/1rcL3RpNgIwTMkS1siPjwHFb2W7jP-AIG/view?usp=sharing)